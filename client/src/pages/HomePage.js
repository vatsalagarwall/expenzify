...
//             console.log(res.data) // Display transactions in the console
//         } catch (error) {
//             console.log(error)
//             message.error('Some Issue with transaction Fetching')
//         }
//     }
...
const HomePage = () => {
    ...
    const getAllTransactions = async () => {
        try {
            const user = JSON.parse(localStorage.getItem("user"));
            setLoading(true);
            const response = await axios.post('/api/v1/transactions/get-transaction', { userid: user._id });
            setLoading(false);
            setAllTransaction(response.data);
            console.log(response.data); // Display transactions in the console
        } catch (error) {
            console.log(error);
            message.error('Some Issue with transaction Fetching');
            setLoading(false);
